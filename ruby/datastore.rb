...
class DataObject
  ...
  def to_json(state=nil, unknown=nil)
  ...
  def self.set(*a)
    ...
    a.each {|m|
      ...
      define_method("#{m}=") {|val|
        raise ArgumentError.new("#{val} is not a set value") unless val.nil? or val.is_a?(Array) or val.is_a?(Set)
        instance_variable_set("@#{m}", val.to_set)
      }
    }
  end
